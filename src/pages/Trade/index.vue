<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressList" :key="address.id" @click="changeDefault(address)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="trade in tradeList.detailArrayList" :key="trade.id">
          <li>
            <img style="hei: 100px;width: 100px;" :src="trade.imgUrl" alt="">
          </li>
          <li>
            <p>
              {{trade.skuName}}
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{defalutAddress.userAddress}}</span>
        收货人：
        <span>{{defalutAddress.consignee}}</span>
        电话：
        <span>{{defalutAddress.phoneNum}}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <a class="subBtn" herf="#none" @click="submitOrder">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.defalutAddress.consignee,
        consigneeTel: this.defalutAddress.phoneNum,
        deliveryAddres: this.defalutAddress.userAddress,
        paymentWay: 'ONLINE',
        orderComment: this.msg,
        orderDetailList: this.tradeList.orderDetailVoList[0].orderDetailList,
      }
